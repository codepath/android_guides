...
An Android device with Google Play Services will already have GCM client support available.  For push notifications to be received, an app must first request an [instance ID](https://developers.google.com/instance-id/reference/) and request a token from a Google server.  This token then must be passed along to your server so that it can be used to send subsequent push notifications.  
