...
You may also want to [re-install Java 7](http://www.oracle.com/technetwork/java/javase/downloads/jdk7-downloads-1880260.html) from this location and retry again and then if still brokwn **uninstall your existing ADT setup**, [re-download the ADT bundle](http://developer.android.com/sdk/index.html) again, and then update all packages from scratch on the new installation. Restart the new eclipse and often this will resolve any issues.
...
First, try simply running "Project...Clean" from the top-menu on the project and then re-running. If that doesn't work, it might be the issue explained in the next paragraph.
