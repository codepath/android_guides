...
Monitoring network traffic to help with troubleshooting depends on the networking library you use.  If you are using [[OkHttp|Using-OkHttp]] or [[Retrofit|Consuming-APIs-with-Retrofit]], you can include [[Stetho|Debugging with Stetho]] project.   This project allows you to use the Chrome network traffic inspector.

If are you are using [[Android Async Http Client|Using-Android-Async-Http-Client]], you cannot leverage the Stetho project and Chrome project as noted in this [issue](https://github.com/facebook/stetho/issues/116).  However, you can still setup an HTTP proxy that can intercept the network requests.  The process requires two parts: one on the PC that will act as the proxy, and the other on the Android device.
