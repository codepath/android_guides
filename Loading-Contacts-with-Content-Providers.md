...
The two different methods are outlined below. In first approach, the loader manager is used to fetch the cursor asynchronously and the cursor is loaded directly into a `SimpleCursorAdapter`. In the [[the second|Loading-Contacts-with-Content-Providers#loading-data-from-cursorloader-into-java-objects]], the content provider data for contacts is loaded synchronously and the cursor is iterated manually into an `ArrayList<Contact>` objects. 
