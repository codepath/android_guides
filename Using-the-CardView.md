...
Android 5.0 introduces a new widget called `CardView` which essentially can be thought of as a `FrameLayout` with rounded corners and shadow based on its elevation. Note that a CardView wraps a single item layout but will often be the container in the layout for each item within a `ListView` or [[RecyclerView|Using-the-RecyclerView]].
