...
In both cases, the information needed to implement the scrolling include the index of the first visible item within the list, the total number of visible items, and some type of threshold value to start fetching more data before the last item has been reached.  This data can be used to decide when to load more data from an external source:
...
public abstract class EndlessRecyclerViewScrollListener extends RecyclerView.OnScrollListener {
    ...
    public void onScrolled(RecyclerView view, int dx, int dy) {
        ...
        if (!loading && (maxSize + visibleThreshold * mStaggeredGridLayoutManager.getSpanCount()) > totalItemCount) {
            currentPage++;
            onLoadMore(currentPage, totalItemCount);
            loading = true;
        }
