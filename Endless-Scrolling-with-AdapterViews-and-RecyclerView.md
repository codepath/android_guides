...
In both cases, the information needed to implement the scrolling include the last visible item within the list and some type of threshold value to start fetching more data before the last item has been reached.  This data can be used to decide when to load more data from an external source:
