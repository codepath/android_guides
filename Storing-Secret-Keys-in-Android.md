...
A public/private RSA key pair is generated, which is stored in the Android device's keystore and protected usually by the device PIN.  An AES symmetric key is also generated, which is used to encrypt and decrypt the secrets.  The app needs to store this AES symmetric key to later decode, so it is encrypted by the RSA public key first before persisted.  When the app runs, it gives this encrypted AES key to the Keystore API, which will decode the data using its private RSA key.  In this way, data cannot be decoded without the use of this encrypted key, which can only be decoded using the device's keystore.
