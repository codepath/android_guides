...
Add a folder called `drawable` under the `res/` folder.
...
**Note:** The style `white_background_wh` uses this built-in [nine patch dialog 9-patch image](https://github.com/inteist/android-better-time-picker/blob/master/res/drawable-hdpi/dialog_full_holo_light.9.png) called `dialog_full_holo_light.9.png` to achieve the border shadow for the layout. Alternatively, you can create simpler box shadows using [[layer-lists|Drawables#creating-a-layer-list]].
